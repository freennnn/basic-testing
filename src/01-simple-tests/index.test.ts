...
describe('simpleCalculator tests', () => {
  ...
  test('should return null for invalid action', () => {
    expect(
      simpleCalculator({ a: 3, b: 3, action: 'Do I look like an action?' }),
    ).toBe(null);
  });

  test('should return null for invalid arguments', () => {
    expect(
      simpleCalculator({ a: 'A', b: 'B', action: 'Sideli na trube' }),
    ).toBeNull();
    expect(
      simpleCalculator({ a: 'A', b: 2, action: Action.Divide }),
    ).toBeNull();
    expect(
      simpleCalculator({ a: 2, b: 'B', action: Action.Multiply }),
    ).toBeNull();
  });
});
